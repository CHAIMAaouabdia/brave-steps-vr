...
function SessionPage() {
  ...
  const finish = () => {
    ...
    toast.success(t("sess.levelSuccessToast", { id: level.id, xp: level.xp }));
  };
    ...
    return (
      <div className="relative grid min-h-screen place-items-center overflow-hidden">
        ...
        <div className="relative z-10 mx-4 max-w-lg rounded-4xl bg-card p-8 text-center shadow-float">
          ...
          <Badge className="mt-4 rounded-full" variant="secondary">{t("sess.level")} {level.id} · {level.difficulty}</Badge>
          ...
          <div className="mt-6 flex flex-wrap justify-center gap-3">
            ...
            >
              <Glasses className="size-4" /> {t("sess.enterVR")}
            </Button>
            <Button asChild size="lg" variant="outline" className="rounded-2xl">
              <Link to="/therapy">{t("sess.back")}</Link>
            </Button>
          </div>
      ...
      <div className="relative z-10 flex min-h-screen flex-col p-4 sm:p-8">
        <div className="grid grid-cols-[minmax(0,1fr)_auto] items-center gap-3">
          <div className="min-w-0 rounded-3xl bg-card/85 px-4 py-3 backdrop-blur">
            <p className="truncate text-xs text-muted-foreground">{t("sess.title", { id: level.id, title: level.title })}</p>
          ...
          <div className="flex shrink-0 gap-2">
            <Button variant="secondary" className="rounded-2xl" onClick={() => setRunning((r) => !r)}>
              {running ? <><Pause className="size-4" /> {t("sess.pause")}</> : <><Play className="size-4" /> {t("sess.resume")}</>}
            </Button>
            <Button
              variant="secondary"
            ...
            >
              <LogOut className="size-4" /> {t("sess.quit")}
            </Button>
          </div>
        </div>
        ...
        <div className="mt-auto grid gap-4 lg:grid-cols-3">
          <div className="rounded-4xl bg-card/90 p-5 backdrop-blur shadow-float">
            <p className="text-xs text-muted-foreground">{t("sess.timer")}</p>
            <p className="text-4xl font-extrabold tabular-nums">{mmss(Math.min(elapsed, total))}</p>
            <p className="text-xs text-muted-foreground">{t("sess.targetTime", { duration: level.duration })}</p>
            <Progress value={pct} className="mt-4 h-2.5" />
            <p className="mt-2 text-xs text-muted-foreground">{t("sess.percentDone", { pct })}</p>
          </div>

          <div className="rounded-4xl bg-card/90 p-5 backdrop-blur shadow-float">
            <p className="mb-3 text-xs text-muted-foreground">{t("sess.objectives")}</p>
          ...
          <div className="rounded-4xl bg-card/90 p-5 backdrop-blur shadow-float">
            {done ? (
              <div className="text-center">
                ...
                <p className="mt-2 font-bold">{t("sess.missionSuccess")}</p>
                <p className="text-xs text-muted-foreground">
                  {t("sess.rewards", { xp: level.xp, coins: level.coins, reward: level.reward })}
                </p>
                <div className="mt-4 flex flex-col gap-2">
                  <Button asChild className="gradient-primary rounded-2xl">
                    <Link to="/therapy">{t("sess.nextLevelUnlocked")}</Link>
                  </Button>
                  <Button asChild variant="outline" className="rounded-2xl">
                    <Link to="/dashboard">{t("sess.dashboard")}</Link>
                  </Button>
                </div>
              </div>
            ) : (
              <>
                <p className="text-xs text-muted-foreground">{t("sess.stressLevel")}</p>
                <p className="text-3xl font-extrabold text-emerald">
                  {Math.max(2, 8 - Math.round(pct / 15))}/10
                </p>
                <p className="mt-1 text-xs text-muted-foreground">
                  {t("sess.breathingHint")}
                </p>
                <Button className="gradient-primary mt-4 w-full rounded-2xl" onClick={finish}>
                  <CheckCircle2 className="size-4" /> {t("sess.finishSession")}
                </Button>
              </>
            )}
          </div>
